...
1. Onboard one primary payment provider (Paystack) → merchant approval →
   product/plan IDs → webhook secret.
2. Stitch (or equivalent) merchant onboarding for debit-order/DebiCheck (post-beta).
3. Configure webhook endpoint, secret validation, failed-payment handling,
   cancellation, receipts/reconciliation.
4. Sandbox-test: monthly card, annual card, debit-order mandate, failed payment,
   retry, cancellation, beta expiry — before enabling real checkout (the full
   go-live gate is in [SECURITY.md](SECURITY.md)).
...
Cloud Run deploy) is tracked in [STATUS.md](STATUS.md).
