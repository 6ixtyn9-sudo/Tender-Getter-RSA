...
def test_orbit_tvet_source_initialization():
...
def test_orbit_tvet_parse_mock_html():
    from tender_getter.sources.tvet.orbit_tvet import OrbitTvetSource, MOCK_HTML
    src = OrbitTvetSource()
    tenders = src.parse_html(MOCK_HTML)
    assert len(tenders) >= 2


def test_orbit_tvet_fetch_uses_fallback_on_empty():
    from tender_getter.sources.tvet.orbit_tvet import OrbitTvetSource
    src = OrbitTvetSource()
