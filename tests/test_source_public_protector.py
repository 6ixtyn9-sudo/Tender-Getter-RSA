...
def test_public_protector_source_initialization():
...
def test_public_protector_parse_mock_html():
    from tender_getter.sources.chapter9.public_protector import PublicProtectorSource, MOCK_HTML
    src = PublicProtectorSource()
    tenders = src.parse_html(MOCK_HTML)
    assert len(tenders) >= 2


def test_public_protector_fetch_uses_fallback_on_empty():
    from tender_getter.sources.chapter9.public_protector import PublicProtectorSource
    src = PublicProtectorSource()
