...
def test_gaming_board_source_initialization():
...
def test_gaming_board_parse_mock_html():
    from tender_getter.sources.regulators.gaming_board import GamingBoardSource, MOCK_HTML
    src = GamingBoardSource()
    tenders = src.parse_html(MOCK_HTML)
    assert len(tenders) >= 2


def test_gaming_board_fetch_uses_fallback_on_empty():
    from tender_getter.sources.regulators.gaming_board import GamingBoardSource
    src = GamingBoardSource()
