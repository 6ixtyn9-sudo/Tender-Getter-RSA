...
def test_pansalb_source_initialization():
...
def test_pansalb_parse_mock_html():
    from tender_getter.sources.chapter9.pansalb import PansalbSource, MOCK_HTML
    src = PansalbSource()
    tenders = src.parse_html(MOCK_HTML)
    assert len(tenders) >= 2


def test_pansalb_fetch_uses_fallback_on_empty():
    from tender_getter.sources.chapter9.pansalb import PansalbSource
    src = PansalbSource()
