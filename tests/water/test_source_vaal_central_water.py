"""Tests for the Vaal Central Water (merged: Bloem + Sedibeng + Lepelle) tender source plug-in."""
import pytest


def test_vaal_central_water_source_initialization():
    from tender_getter.sources.water.vaal_central_water import VaalCentralWaterSource
    src = VaalCentralWaterSource()
    assert src.source_id == "vaal_central_water"
    assert isinstance(src.live, bool)


def test_vaal_central_water_parse_mock_html():
    from tender_getter.sources.water.vaal_central_water import VaalCentralWaterSource, MOCK_HTML
    src = VaalCentralWaterSource()
    tenders = src.parse_html(MOCK_HTML)
    assert len(tenders) >= 2


def test_vaal_central_water_fetch_uses_fallback_on_empty():
    from tender_getter.sources.water.vaal_central_water import VaalCentralWaterSource
    src = VaalCentralWaterSource()
    tenders = src.fetch(html_content="<html><body>no tenders here</body></html>")
    assert len(tenders) >= 2
