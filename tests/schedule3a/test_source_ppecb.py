...
def test_ppecb_source_initialization():
...
def test_ppecb_parse_mock_html():
    from tender_getter.sources.schedule3a.ppecb import PpecbSource, MOCK_HTML
    src = PpecbSource()
    tenders = src.parse_html(MOCK_HTML)
    assert len(tenders) >= 2


def test_ppecb_fetch_uses_fallback_on_empty():
    from tender_getter.sources.schedule3a.ppecb import PpecbSource
    src = PpecbSource()
