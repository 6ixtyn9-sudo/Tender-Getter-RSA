...
def test_etdp_seta_source_initialization():
...
def test_etdp_seta_parse_mock_html():
    from tender_getter.sources.setas.etdp_seta import EtdpSetaSource, MOCK_HTML
    src = EtdpSetaSource()
    tenders = src.parse_html(MOCK_HTML)
    assert len(tenders) >= 2


def test_etdp_seta_fetch_uses_fallback_on_empty():
    from tender_getter.sources.setas.etdp_seta import EtdpSetaSource
    src = EtdpSetaSource()
