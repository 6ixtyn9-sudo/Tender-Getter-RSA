...
def test_lp_tourism_source_initialization():
...
def test_lp_tourism_parse_mock_html():
    from tender_getter.sources.research.lp_tourism import LpTourismSource, MOCK_HTML
    src = LpTourismSource()
    tenders = src.parse_html(MOCK_HTML)
    assert len(tenders) >= 2


def test_lp_tourism_fetch_uses_fallback_on_empty():
    from tender_getter.sources.research.lp_tourism import LpTourismSource
    src = LpTourismSource()
