...
def test_mp_health_source_initialization():
...
def test_mp_health_parse_mock_html():
    from tender_getter.sources.research.mp_health import MpHealthSource, MOCK_HTML
    src = MpHealthSource()
    tenders = src.parse_html(MOCK_HTML)
    assert len(tenders) >= 2


def test_mp_health_fetch_uses_fallback_on_empty():
    from tender_getter.sources.research.mp_health import MpHealthSource
    src = MpHealthSource()
