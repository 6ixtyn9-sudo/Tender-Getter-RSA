"""Tests for the Ayurveda Council tender source plug-in."""
import pytest


def test_ayurveda_council_source_initialization():
    from tender_getter.sources.research.ayurveda_council import AyurvedaCouncilSource
    src = AyurvedaCouncilSource()
    assert src.source_id == "ayurveda_council"
    assert isinstance(src.live, bool)


def test_ayurveda_council_parse_mock_html():
    from tender_getter.sources.research.ayurveda_council import AyurvedaCouncilSource, MOCK_HTML
    src = AyurvedaCouncilSource()
    tenders = src.parse_html(MOCK_HTML)
    assert len(tenders) >= 2


def test_ayurveda_council_fetch_uses_fallback_on_empty():
    from tender_getter.sources.research.ayurveda_council import AyurvedaCouncilSource
    src = AyurvedaCouncilSource()
    tenders = src.fetch(html_content="<html><body>no tenders here</body></html>")
    assert len(tenders) >= 2
