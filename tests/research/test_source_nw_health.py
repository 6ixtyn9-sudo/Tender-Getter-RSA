...
def test_nw_health_source_initialization():
...
def test_nw_health_parse_mock_html():
    from tender_getter.sources.research.nw_health import NwHealthSource, MOCK_HTML
    src = NwHealthSource()
    tenders = src.parse_html(MOCK_HTML)
    assert len(tenders) >= 2


def test_nw_health_fetch_uses_fallback_on_empty():
    from tender_getter.sources.research.nw_health import NwHealthSource
    src = NwHealthSource()
