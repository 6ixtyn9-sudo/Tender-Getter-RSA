...
def test_agriseta_source_initialization():
...
def test_agriseta_parse_mock_html():
    from tender_getter.sources.setas.agriseta import AgrisetaSource, MOCK_HTML
    src = AgrisetaSource()
    tenders = src.parse_html(MOCK_HTML)
    assert len(tenders) >= 2


def test_agriseta_fetch_uses_fallback_on_empty():
    from tender_getter.sources.setas.agriseta import AgrisetaSource
    src = AgrisetaSource()
