...
def test_fasset_source_initialization():
...
def test_fasset_parse_mock_html():
    from tender_getter.sources.setas.fasset import FassetSource, MOCK_HTML
    src = FassetSource()
    tenders = src.parse_html(MOCK_HTML)
    assert len(tenders) >= 2


def test_fasset_fetch_uses_fallback_on_empty():
    from tender_getter.sources.setas.fasset import FassetSource
    src = FassetSource()
