...
def test_zamukulungisa_lm_source_initialization():
...
def test_zamukulungisa_lm_parse_mock_html():
    from tender_getter.sources.local_municipalities.zamukulungisa_lm import ZamukulungisaLmSource, MOCK_HTML
    src = ZamukulungisaLmSource()
    tenders = src.parse_html(MOCK_HTML)
    assert len(tenders) >= 2


def test_zamukulungisa_lm_fetch_uses_fallback_on_empty():
    from tender_getter.sources.local_municipalities.zamukulungisa_lm import ZamukulungisaLmSource
    src = ZamukulungisaLmSource()
