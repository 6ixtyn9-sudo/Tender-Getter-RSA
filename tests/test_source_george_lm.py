...
def test_george_lm_parse_mock_html():
    from tender_getter.sources.local_municipalities.george_lm import GeorgeLmSource, MOCK_HTML
    src = GeorgeLmSource()
    tenders = src.parse_html(MOCK_HTML)
    assert len(tenders) >= 2


def test_george_lm_fetch_uses_fallback_on_empty():
    from tender_getter.sources.local_municipalities.george_lm import GeorgeLmSource
    src = GeorgeLmSource()
