...
def test_mandela_uni_source_initialization():
...
def test_mandela_uni_parse_mock_html():
    from tender_getter.sources.universities.mandela_uni import MandelaUniSource, MOCK_HTML
    src = MandelaUniSource()
    tenders = src.parse_html(MOCK_HTML)
    assert len(tenders) >= 2


def test_mandela_uni_fetch_uses_fallback_on_empty():
    from tender_getter.sources.universities.mandela_uni import MandelaUniSource
    src = MandelaUniSource()
