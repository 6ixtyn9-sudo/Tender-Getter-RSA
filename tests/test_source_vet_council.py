...
def test_vet_council_source_initialization():
...
def test_vet_council_parse_mock_html():
    from tender_getter.sources.research.vet_council import VetCouncilSource, MOCK_HTML
    src = VetCouncilSource()
    tenders = src.parse_html(MOCK_HTML)
    assert len(tenders) >= 2


def test_vet_council_fetch_uses_fallback_on_empty():
    from tender_getter.sources.research.vet_council import VetCouncilSource
    src = VetCouncilSource()
