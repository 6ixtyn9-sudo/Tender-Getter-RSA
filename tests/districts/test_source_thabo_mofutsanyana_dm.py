"""Tests for the Thabo Mofutsanyana District Municipality tender source plug-in."""
import pytest


def test_thabo_mofutsanyana_dm_source_initialization():
    from tender_getter.sources.districts.thabo_mofutsanyana_dm import ThaboMofutsanyanaDmSource
    src = ThaboMofutsanyanaDmSource()
    assert src.source_id == "thabo_mofutsanyana_dm"
    assert isinstance(src.live, bool)


def test_thabo_mofutsanyana_dm_parse_mock_html():
    from tender_getter.sources.districts.thabo_mofutsanyana_dm import ThaboMofutsanyanaDmSource, MOCK_HTML
    src = ThaboMofutsanyanaDmSource()
    tenders = src.parse_html(MOCK_HTML)
    assert len(tenders) >= 2


def test_thabo_mofutsanyana_dm_fetch_uses_fallback_on_empty():
    from tender_getter.sources.districts.thabo_mofutsanyana_dm import ThaboMofutsanyanaDmSource
    src = ThaboMofutsanyanaDmSource()
    tenders = src.fetch(html_content="<html><body>no tenders here</body></html>")
    assert len(tenders) >= 2
