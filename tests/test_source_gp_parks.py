...
def test_gp_parks_source_initialization():
...
def test_gp_parks_parse_mock_html():
    from tender_getter.sources.research.gp_parks import GpParksSource, MOCK_HTML
    src = GpParksSource()
    tenders = src.parse_html(MOCK_HTML)
    assert len(tenders) >= 2


def test_gp_parks_fetch_uses_fallback_on_empty():
    from tender_getter.sources.research.gp_parks import GpParksSource
    src = GpParksSource()
