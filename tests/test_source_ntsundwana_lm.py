...
def test_ntsundwana_lm_source_initialization():
...
def test_ntsundwana_lm_parse_mock_html():
    from tender_getter.sources.local_municipalities.ntsundwana_lm import NtsundwanaLmSource, MOCK_HTML
    src = NtsundwanaLmSource()
    tenders = src.parse_html(MOCK_HTML)
    assert len(tenders) >= 2


def test_ntsundwana_lm_fetch_uses_fallback_on_empty():
    from tender_getter.sources.local_municipalities.ntsundwana_lm import NtsundwanaLmSource
    src = NtsundwanaLmSource()
