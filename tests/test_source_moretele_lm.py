...
def test_moretele_lm_source_initialization():
...
def test_moretele_lm_parse_mock_html():
    from tender_getter.sources.local_municipalities.moretele_lm import MoreteleLmSource, MOCK_HTML
    src = MoreteleLmSource()
    tenders = src.parse_html(MOCK_HTML)
    assert len(tenders) >= 2


def test_moretele_lm_fetch_uses_fallback_on_empty():
    from tender_getter.sources.local_municipalities.moretele_lm import MoreteleLmSource
    src = MoreteleLmSource()
