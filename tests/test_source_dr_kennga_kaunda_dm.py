...
def test_dr_kennga_kaunda_dm_source_initialization():
...
def test_dr_kennga_kaunda_dm_parse_mock_html():
    from tender_getter.sources.districts.dr_kennga_kaunda_dm import DrKenngaKaundaDmSource, MOCK_HTML
    src = DrKenngaKaundaDmSource()
    tenders = src.parse_html(MOCK_HTML)
    assert len(tenders) >= 2


def test_dr_kennga_kaunda_dm_fetch_uses_fallback_on_empty():
    from tender_getter.sources.districts.dr_kennga_kaunda_dm import DrKenngaKaundaDmSource
    src = DrKenngaKaundaDmSource()
