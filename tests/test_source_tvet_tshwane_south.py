...
def test_tvet_tshwane_south_source_initialization():
...
def test_tvet_tshwane_south_parse_mock_html():
    from tender_getter.sources.tvet.tvet_tshwane_south import TvetTshwaneSouthSource, MOCK_HTML
    src = TvetTshwaneSouthSource()
    tenders = src.parse_html(MOCK_HTML)
    assert len(tenders) >= 2


def test_tvet_tshwane_south_fetch_uses_fallback_on_empty():
    from tender_getter.sources.tvet.tvet_tshwane_south import TvetTshwaneSouthSource
    src = TvetTshwaneSouthSource()
