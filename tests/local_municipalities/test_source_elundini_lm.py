...
def test_elundini_lm_source_initialization():
...
def test_elundini_lm_parse_mock_html():
    from tender_getter.sources.local_municipalities.elundini_lm import ElundiniLmSource, MOCK_HTML
    src = ElundiniLmSource()
    tenders = src.parse_html(MOCK_HTML)
    assert len(tenders) >= 2


def test_elundini_lm_fetch_uses_fallback_on_empty():
    from tender_getter.sources.local_municipalities.elundini_lm import ElundiniLmSource
    src = ElundiniLmSource()
