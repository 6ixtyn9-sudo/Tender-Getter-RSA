...
def test_mahlangeni_lm_source_initialization():
...
def test_mahlangeni_lm_parse_mock_html():
    from tender_getter.sources.local_municipalities.mahlangeni_lm import MahlangeniLmSource, MOCK_HTML
    src = MahlangeniLmSource()
    tenders = src.parse_html(MOCK_HTML)
    assert len(tenders) >= 2


def test_mahlangeni_lm_fetch_uses_fallback_on_empty():
    from tender_getter.sources.local_municipalities.mahlangeni_lm import MahlangeniLmSource
    src = MahlangeniLmSource()
