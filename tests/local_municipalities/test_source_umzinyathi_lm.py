...
def test_umzinyathi_lm_source_initialization():
...
def test_umzinyathi_lm_parse_mock_html():
    from tender_getter.sources.local_municipalities.umzinyathi_lm import UmzinyathiLmSource, MOCK_HTML
    src = UmzinyathiLmSource()
    tenders = src.parse_html(MOCK_HTML)
    assert len(tenders) >= 2


def test_umzinyathi_lm_fetch_uses_fallback_on_empty():
    from tender_getter.sources.local_municipalities.umzinyathi_lm import UmzinyathiLmSource
    src = UmzinyathiLmSource()
