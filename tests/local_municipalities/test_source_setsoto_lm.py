...
def test_setsoto_lm_source_initialization():
...
def test_setsoto_lm_parse_mock_html():
    from tender_getter.sources.local_municipalities.setsoto_lm import SetsotoLmSource, MOCK_HTML
    src = SetsotoLmSource()
    tenders = src.parse_html(MOCK_HTML)
    assert len(tenders) >= 2


def test_setsoto_lm_fetch_uses_fallback_on_empty():
    from tender_getter.sources.local_municipalities.setsoto_lm import SetsotoLmSource
    src = SetsotoLmSource()
