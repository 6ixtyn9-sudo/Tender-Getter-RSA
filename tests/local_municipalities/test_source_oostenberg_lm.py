...
def test_oostenberg_lm_source_initialization():
...
def test_oostenberg_lm_parse_mock_html():
    from tender_getter.sources.local_municipalities.oostenberg_lm import OostenbergLmSource, MOCK_HTML
    src = OostenbergLmSource()
    tenders = src.parse_html(MOCK_HTML)
    assert len(tenders) >= 2


def test_oostenberg_lm_fetch_uses_fallback_on_empty():
    from tender_getter.sources.local_municipalities.oostenberg_lm import OostenbergLmSource
    src = OostenbergLmSource()
