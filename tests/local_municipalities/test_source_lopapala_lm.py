...
def test_lopapala_lm_source_initialization():
...
def test_lopapala_lm_parse_mock_html():
    from tender_getter.sources.local_municipalities.lopapala_lm import LopapalaLmSource, MOCK_HTML
    src = LopapalaLmSource()
    tenders = src.parse_html(MOCK_HTML)
    assert len(tenders) >= 2


def test_lopapala_lm_fetch_uses_fallback_on_empty():
    from tender_getter.sources.local_municipalities.lopapala_lm import LopapalaLmSource
    src = LopapalaLmSource()
