...
def test_mogale_city_lm_source_initialization():
...
def test_mogale_city_lm_parse_mock_html():
    from tender_getter.sources.local_municipalities.mogale_city_lm import MogaleCityLmSource, MOCK_HTML
    src = MogaleCityLmSource()
    tenders = src.parse_html(MOCK_HTML)
    assert len(tenders) >= 2


def test_mogale_city_lm_fetch_uses_fallback_on_empty():
    from tender_getter.sources.local_municipalities.mogale_city_lm import MogaleCityLmSource
    src = MogaleCityLmSource()
