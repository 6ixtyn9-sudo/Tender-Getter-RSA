...
def test_food_agency_source_initialization():
...
def test_food_agency_parse_mock_html():
    from tender_getter.sources.regulators.food_agency import FoodAgencySource, MOCK_HTML
    src = FoodAgencySource()
    tenders = src.parse_html(MOCK_HTML)
    assert len(tenders) >= 2


def test_food_agency_fetch_uses_fallback_on_empty():
    from tender_getter.sources.regulators.food_agency import FoodAgencySource
    src = FoodAgencySource()
