...
def test_fs_social_source_initialization():
...
def test_fs_social_parse_mock_html():
    from tender_getter.sources.research.fs_social import FsSocialSource, MOCK_HTML
    src = FsSocialSource()
    tenders = src.parse_html(MOCK_HTML)
    assert len(tenders) >= 2


def test_fs_social_fetch_uses_fallback_on_empty():
    from tender_getter.sources.research.fs_social import FsSocialSource
    src = FsSocialSource()
