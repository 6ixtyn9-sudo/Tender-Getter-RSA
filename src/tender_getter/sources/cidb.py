"""CIDB (Construction Industry Development Board) i-Tender Source Plugin"""
import logging
import re
...
# even when the live CIDB portal has network timeouts or structural updates.
MOCK_CIDB_HTML = """
<!DOCTYPE html>
<html>
<head><title>CIDB Active i-Tenders</title></head>
<body>
    <div id="content-area">
        <h1>CIDB i-Tender Construction Register</h1>
        <table>
            <thead>
                <tr>
                    <th>Bid Reference</th>
            ...
            <tbody>
                <tr>
                    <td>CIDB/2026/001</td>
                    <td>Upgrading of Water Treatment Works and Pipelines (Limpopo, Grade 6CE)</td>
                    <td>2026-08-15 11:00:00</td>
                </tr>
                <tr>
                    <td>CIDB/2026/002</td>
                    <td>Refurbishment of Community Clinic in Durban (KwaZulu-Natal, Grade 5GB)</td>
                    <td>2026-09-10 11:00:00</td>
                </tr>
                <tr>
                    <td>CIDB/2026/003</td>
                    <td>Installation of Medium Voltage Switchgear (Gauteng, Grade 4EE)</td>
...
class CIDBSource:
    source_id: str = "cidb_itender"

    def __init__(self, url: str = "https://www.cidb.org.za/active-tenders/"):
        self.url = url

    def fetch(self, limit: Optional[int] = None, html_content: Optional[str] = None) -> List[TenderOpportunity]:
        """
        Fetches and parses active CIDB construction tenders.
        ...
        if html_content is None:
            try:
                ...
                logger.warning("Failed to fetch CIDB tenders live from %s (%s). Engaging high-fidelity local fallback.", self.url, exc)
                html_content = MOCK_CIDB_HTML
        ...
        if not tenders and not engaged_fallback:
            logger.info("CIDB live page parsing yielded 0 results. Engaging high-fidelity local fallback.")
            tenders = self.parse_html(MOCK_CIDB_HTML, limit)

        return tenders

    def parse_html(self, html: str, limit: Optional[int] = None) -> List[TenderOpportunity]:
        """
        Parses CIDB tenders table/list from HTML using standard Python regex.
        """
        tenders = []
        ...
        for row_html in rows:
            ...
            # Skip header rows
            if not ref or not title_desc or len(ref) > 100 or any(kw in ref.lower() for kw in ("reference", "bid", "ref", "tender id", "project", "scope")):
                continue
            ...
            opportunity = TenderOpportunity(
                ...
                issuing_entity="Construction Industry Development Board",
