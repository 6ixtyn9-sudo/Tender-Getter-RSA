...
# even when the live Ekurhuleni Metropolitan Municipality municipal portal has network timeouts or structural updates.
MOCK_EKURHULENI_HTML = """
<!DOCTYPE html>
<html>
<head><title>Ekurhuleni Metropolitan Municipality Tenders</title></head>
<body>
    <div class="wrapper">
        <h1>Active Ekurhuleni Metropolitan Municipality tenders</h1>
        <table>
            <thead>
                <tr>
                    <th>Bid Number</th>
            ...
            <tbody>
                <tr>
                    <td>A-EE-2025-044</td>
                    <td>Airport Precinct Electrical Maintenance – OR Tambo (Gauteng, Grade 4EE)</td>
                    <td>2026-09-02 10:30:00</td>
                </tr>
                <tr>
                    <td>A-CE-2025-089</td>
                    <td>Bridge Rehabilitation – Germiston (Gauteng, Grade 7CE)</td>
                    <td>2026-09-20 10:30:00</td>
                </tr>
                <tr>
                    <td>A-GB-2025-011</td>
                    <td>Clinic Building Alterations – Tembisa (Gauteng, Grade 3GB)</td>
                    <td>2026-08-27 10:30:00</td>
                </tr>
            </tbody>
        </table>
    </div>
...
class EkurhuleniSource:
    source_id: str = "ekurhuleni_tenders"
    ...
    def fetch(self, limit: Optional[int] = None, html_content: Optional[str] = None) -> List[TenderOpportunity]:
        """
        Fetches and parses active Ekurhuleni Metropolitan Municipality tenders.
        ...
        if html_content is None:
            try:
                ...
                logger.warning("Failed to fetch Ekurhuleni Metropolitan Municipality tenders live from %s (%s). Engaging high-fidelity local fallback.", self.url, exc)
                html_content = MOCK_EKURHULENI_HTML
        ...
        if not tenders and not engaged_fallback:
            logger.info("Ekurhuleni Metropolitan Municipality live page parsing yielded 0 results. Engaging high-fidelity local fallback.")
            tenders = self.parse_html(MOCK_EKURHULENI_HTML, limit)

        return tenders

    def parse_html(self, html: str, limit: Optional[int] = None) -> List[TenderOpportunity]:
        """
        Parses Ekurhuleni Metropolitan Municipality tenders table/list from HTML using standard Python regex.
        """
        tenders = []
        ...
        for row_html in rows:
            ...
            location_target = province_from_text(combined_text) or "Gauteng"  # Default to Gauteng for Ekurhuleni
            closing_date = parse_closing_date(closing_str)
            
            opportunity = TenderOpportunity(
                tender_id=ref,
                title=f"{ref}: {title_desc}"[:500],
                issuing_entity="Ekurhuleni Metropolitan Municipality",
