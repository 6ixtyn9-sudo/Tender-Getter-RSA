"""Overberg Water Tender Source Plugin"""
import logging
import re
...
MOCK_OVERBERG_WATER_HTML = """
<!DOCTYPE html>
<html>
<head><title>Overberg Water Tenders</title></head>
<body>
    <div class="wrapper">
        <h1>Active Overberg Water tenders</h1>
        <table>
            <thead>
                <tr>
                    <th>Bid Number</th>
            ...
            <tbody>
                <tr>
                    <td>OW/2025/CE/06</td>
                    <td>Reservoir Construction – Caledon (Western Cape, Grade 3CE)</td>
                    <td>2026-09-08 11:00:00</td>
                </tr>
                <tr>
                    <td>OW/2025/PIPE/02</td>
                    <td>Distribution Pipeline Upgrade (Western Cape, Grade 2CE)</td>
                    <td>2026-08-27 11:00:00</td>
                </tr>
                <tr>
                    <td>OW/2025/MAINT/05</td>
                    <td>Pump Station Maintenance (Western Cape)</td>
...
class OverbergWaterSource:
    source_id: str = "overberg_water_tenders"

    def __init__(self, url: str = "https://overbergwater.co.za/tenders"):
        self.url = url

    def fetch(self, limit: Optional[int] = None, html_content: Optional[str] = None) -> List[TenderOpportunity]:
        """
        Fetches and parses active Overberg Water tenders.
        ...
        if html_content is None:
            try:
                ...
                html_content = MOCK_OVERBERG_WATER_HTML
        ...
        if not tenders and not engaged_fallback:
            ...
            tenders = self.parse_html(MOCK_OVERBERG_WATER_HTML, limit)

        return tenders

    def parse_html(self, html: str, limit: Optional[int] = None) -> List[TenderOpportunity]:
        """
        Parses Overberg Water tenders table/list from HTML using standard Python regex.
        """
        tenders = []
