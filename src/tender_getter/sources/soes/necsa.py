"""South African Nuclear Energy Corporation Tender Source Plugin"""
import logging
import re
...
<!DOCTYPE html>
<html>
<head><title>South African Nuclear Energy Corporation Tenders</title></head>
<body>
    <div class="wrapper">
        <h1>Active South African Nuclear Energy Corporation tenders</h1>
        <table>
            <thead>
                <tr>
                    <th>Bid Number</th>
            ...
            <tbody>
                <tr>
                    <td>NECSA/2025/LAB/06</td>
                    <td>Nuclear Safety Research Equipment (Gauteng)</td>
                    <td>2026-09-07 11:00:00</td>
                </tr>
                <tr>
                    <td>NECSA/2025/HAZ/02</td>
                    <td>Hazardous Chemical Logistics (Gauteng)</td>
                    <td>2026-08-26 11:00:00</td>
                </tr>
                <tr>
                    <td>NECSA/2025/MAINT/13</td>
                    <td>Reactor Facility Maintenance (Gauteng, Grade 5ME)</td>
                    <td>2026-09-18 11:00:00</td>
                </tr>
            </tbody>
        </table>
    </div>
...
class NECSASource:
    ...
    def fetch(self, limit: Optional[int] = None, html_content: Optional[str] = None) -> List[TenderOpportunity]:
        """
        Fetches and parses active South African Nuclear Energy Corporation tenders.
        ...
        if not tenders and not engaged_fallback:
            ...
            tenders = self.parse_html(MOCK_NECSA_HTML, limit)

        return tenders

    def parse_html(self, html: str, limit: Optional[int] = None) -> List[TenderOpportunity]:
        """
        Parses South African Nuclear Energy Corporation tenders table/list from HTML using standard Python regex.
        """
        tenders = []
        ...
        for row_html in rows:
            ...
            opportunity = TenderOpportunity(
                ...
                issuing_entity="South African Nuclear Energy Corporation",
