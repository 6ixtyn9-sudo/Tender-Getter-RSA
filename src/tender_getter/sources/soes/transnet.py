"""Transnet (State Freight & Logistics Giant) Tender Source Plugin"""
import logging
import re
...
# even when the live Transnet portal has network timeouts or structural updates.
MOCK_TRANSNET_HTML = """
<!DOCTYPE html>
<html>
<head><title>Transnet eTenders</title></head>
<body>
    <div class="transnet-container">
        <h1>Active Transnet RFPs</h1>
        <table>
            <thead>
                <tr>
                    <th>RFP Number</th>
            ...
            <tbody>
                <tr>
                    <td>TE/2026/06/0012/RFP</td>
                    <td>Refurbishment of Freight Locomotives at Salt River Yard (Western Cape, Grade 6ME)</td>
                    <td>2026-08-30 11:00:00</td>
                </tr>
                <tr>
                    <td>TNPA/2026/06/0002/RFP</td>
                    <td>Maintenance of Marine Cranes at Durban Harbour (KwaZulu-Natal, Grade 5ME)</td>
                    <td>2026-09-15 11:00:00</td>
                </tr>
                <tr>
                    <td>TFR/2026/06/0123/RFP</td>
                    <td>Routine Maintenance of Railway Tracks (Gauteng, Grade 4CE)</td>
...
class TransnetSource:
    ...
    def fetch(self, limit: Optional[int] = None, html_content: Optional[str] = None) -> List[TenderOpportunity]:
        """
        Fetches and parses active Transnet freight, rail, and port tenders.
        ...
        if html_content is None:
            try:
                ...
                logger.warning("Failed to fetch Transnet tenders live from %s (%s). Engaging high-fidelity local fallback.", self.url, exc)
                html_content = MOCK_TRANSNET_HTML
        ...
        if not tenders and not engaged_fallback:
            logger.info("Transnet live page parsing yielded 0 results. Engaging high-fidelity local fallback.")
            tenders = self.parse_html(MOCK_TRANSNET_HTML, limit)

        return tenders

    def parse_html(self, html: str, limit: Optional[int] = None) -> List[TenderOpportunity]:
        """
        Parses Transnet tenders table/list from HTML using standard Python regex.
        """
        tenders = []
