"""Water Research Commission Tender Source Plugin"""
import logging
import re
...
MOCK_WRC_HTML = """
<!DOCTYPE html><html><head><title>Water Research Commission Tenders</title></head><body><div class="wrapper"><h1>Active Water Research Commission tenders</h1><table><thead><tr><th>Bid Number</th><th>Bid Description</th><th>Closing Date</th></tr></thead><tbody>
                <tr>
                    <td>WRC/2025/RES/07</td>
                    <td>Water Preservation Research Grant (Gauteng)</td>
                    <td>2026-09-09 11:00:00</td>
                </tr>
                <tr>
                    <td>WRC/2025/CONS/03</td>
                    <td>Hydrology Research Consultancy (Gauteng)</td>
                    <td>2026-08-26 11:00:00</td>
                </tr>
                <tr>
                    <td>WRC/2025/EQ/11</td>
                    <td>Water Quality Monitoring Equipment (Gauteng)</td>
...
class WRCSource:
    ...
    def fetch(self, limit: Optional[int] = None, html_content: Optional[str] = None) -> List[TenderOpportunity]:
        """
        Fetches and parses active Water Research Commission tenders.
        ...
        if not tenders and not engaged_fallback:
            ...
            tenders = self.parse_html(MOCK_WRC_HTML, limit)

        return tenders

    def parse_html(self, html: str, limit: Optional[int] = None) -> List[TenderOpportunity]:
        """
        Parses Water Research Commission tenders table/list from HTML using standard Python regex.
        """
        tenders = []
        ...
        for row_html in rows:
            ...
            opportunity = TenderOpportunity(
                ...
                issuing_entity="Water Research Commission",
