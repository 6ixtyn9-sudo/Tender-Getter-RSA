"""National Health Laboratory Service Tender Source Plugin"""
import logging
import re
...
MOCK_NHLS_HTML = """
<!DOCTYPE html><html><head><title>National Health Laboratory Service Tenders</title></head><body><div class="wrapper"><h1>Active National Health Laboratory Service tenders</h1><table><thead><tr><th>Bid Number</th><th>Bid Description</th><th>Closing Date</th></tr></thead><tbody>
                <tr>
                    <td>NHLS/2025/EQ/22</td>
                    <td>Clinical Diagnostic Equipment (National)</td>
                    <td>2026-09-12 11:00:00</td>
                </tr>
                <tr>
                    <td>NHLS/2025/SUPP/05</td>
                    <td>Laboratory Consumables Supply (National)</td>
                    <td>2026-08-28 11:00:00</td>
                </tr>
                <tr>
                    <td>NHLS/2025/MOB/09</td>
                    <td>Mobile Clinic Vehicles (National)</td>
...
class NHLSSource:
    ...
    def fetch(self, limit: Optional[int] = None, html_content: Optional[str] = None) -> List[TenderOpportunity]:
        """
        Fetches and parses active National Health Laboratory Service tenders.
        ...
        if not tenders and not engaged_fallback:
            ...
            tenders = self.parse_html(MOCK_NHLS_HTML, limit)

        return tenders

    def parse_html(self, html: str, limit: Optional[int] = None) -> List[TenderOpportunity]:
        """
        Parses National Health Laboratory Service tenders table/list from HTML using standard Python regex.
        """
        tenders = []
        ...
        for row_html in rows:
            ...
            opportunity = TenderOpportunity(
                ...
                issuing_entity="National Health Laboratory Service",
