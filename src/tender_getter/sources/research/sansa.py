"""South African National Space Agency Tender Source Plugin"""
import logging
import re
...
MOCK_SANSA_HTML = """
<!DOCTYPE html><html><head><title>South African National Space Agency Tenders</title></head><body><div class="wrapper"><h1>Active South African National Space Agency tenders</h1><table><thead><tr><th>Bid Number</th><th>Bid Description</th><th>Closing Date</th></tr></thead><tbody>
                <tr>
                    <td>SANSA/2025/SPACE/11</td>
                    <td>Satellite Tracking System (Gauteng)</td>
                    <td>2026-09-14 11:00:00</td>
                </tr>
                <tr>
                    <td>SANSA/2025/SOFT/04</td>
                    <td>Remote Sensing Software (Gauteng)</td>
                    <td>2026-08-29 11:00:00</td>
                </tr>
                <tr>
                    <td>SANSA/2025/EQ/07</td>
                    <td>Space Science Instrumentation (Gauteng)</td>
...
class SANSASource:
    source_id: str = "sansa_tenders"

    def __init__(self, url: str = "https://www.sansa.org.za/tenders"):
        self.url = url

    def fetch(self, limit: Optional[int] = None, html_content: Optional[str] = None) -> List[TenderOpportunity]:
        """
        Fetches and parses active South African National Space Agency tenders.
        ...
        if not tenders and not engaged_fallback:
            ...
            tenders = self.parse_html(MOCK_SANSA_HTML, limit)

        return tenders

    def parse_html(self, html: str, limit: Optional[int] = None) -> List[TenderOpportunity]:
        """
        Parses South African National Space Agency tenders table/list from HTML using standard Python regex.
        """
        tenders = []
        ...
        for row_html in rows:
            ...
            opportunity = TenderOpportunity(
                ...
                issuing_entity="South African National Space Agency",
