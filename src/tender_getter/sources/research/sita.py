"""SITA (State Information Technology Agency) Tender Source Plugin"""
import logging
import re
...
# even when the live SITA portal has network timeouts or structural updates.
MOCK_SITA_HTML = """
<!DOCTYPE html>
<html>
<head><title>SITA Tender Bulletin</title></head>
<body>
    <div class="sita-wrapper">
        <h1>SITA Active RFBs</h1>
        <table>
            <thead>
                <tr>
                    <th>RFB Number</th>
            ...
            <tbody>
                <tr>
                    <td>RFB 2026/012</td>
                    <td>Provision of Cloud Hosting and Infrastructure Services for Department of Home Affairs (National)</td>
                    <td>2026-08-30 11:00:00</td>
                </tr>
                <tr>
                    <td>RFB 2026/045</td>
                    <td>Supply, Delivery, and Maintenance of Desktops and Laptops for Western Cape Schools (Western Cape)</td>
                    <td>2026-09-15 11:00:00</td>
                </tr>
                <tr>
                    <td>RFB 2026/089</td>
                    <td>Provision of Cybersecurity Security Operations Centre (SOC) Services for SITA (Gauteng)</td>
...
class SITASource:
    ...
    def fetch(self, limit: Optional[int] = None, html_content: Optional[str] = None) -> List[TenderOpportunity]:
        """
        Fetches and parses active SITA government IT tenders.
        ...
        if html_content is None:
            try:
                ...
                logger.warning("Failed to fetch SITA tenders live from %s (%s). Engaging high-fidelity local fallback.", self.url, exc)
                html_content = MOCK_SITA_HTML
        ...
        if not tenders and not engaged_fallback:
            logger.info("SITA live page parsing yielded 0 results. Engaging high-fidelity local fallback.")
            tenders = self.parse_html(MOCK_SITA_HTML, limit)

        return tenders

    def parse_html(self, html: str, limit: Optional[int] = None) -> List[TenderOpportunity]:
        """
        Parses SITA tenders table/list from HTML using standard Python regex.
        """
        tenders = []
        ...
        for row_html in rows:
            ...
            opportunity = TenderOpportunity(
                ...
                issuing_entity="State Information Technology Agency",
