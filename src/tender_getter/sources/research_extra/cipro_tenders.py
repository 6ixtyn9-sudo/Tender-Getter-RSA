"""CIPRO (legacy CIPC) tender source plug-in."""
import logging
import re
...
<!DOCTYPE html>
<html>
<head><title>CIPRO (legacy CIPC) Tenders</title></head>
<body>
    <div class="tenders-wrapper">
        <h1>Active CIPRO (legacy CIPC) tenders</h1>
        <table>
            <thead>
...
class CiproSource:
    """Tender source plug-in for CIPRO (legacy CIPC)."""

    source_id: str = "cipro_tenders"
    live: bool = False

    def __init__(self, url: str = "https://www.cipro.co.za/procurement"):
        self.url = url
        self.issuing_entity = "CIPRO (legacy CIPC)"
