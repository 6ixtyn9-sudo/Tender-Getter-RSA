"""Northern Cape Department of Roads and Public Works Tender Source Plugin"""
import logging
import re
...
# even when the live Northern Cape municipal portal has network timeouts or structural updates.
MOCK_NC_ROADS_HTML = """
<!DOCTYPE html><html><head><title>Northern Cape Department of Roads and Public Works Tenders</title></head><body><div class="wrapper"><h1>Active Northern Cape Department of Roads and Public Works tenders</h1><table><thead><tr><th>Bid Number</th><th>Bid Description</th><th>Closing Date</th></tr></thead><tbody>
                <tr>
                    <td>NC_R/2025/001</td>
                    <td>Infrastructure Maintenance – Northern Cape (Northern Cape, Grade 3CE)</td>
                    <td>2026-09-10 11:00:00</td>
                </tr>
                <tr>
                    <td>NC_R/2025/002</td>
                    <td>Building Refurbishment – Northern Cape (Northern Cape, Grade 2GB)</td>
                    <td>2026-08-28 11:00:00</td>
                </tr>
                <tr>
                    <td>NC_R/2025/003</td>
                    <td>Professional Services Panel (Northern Cape)</td>
...
class NCRoadsSource:
    ...
    def fetch(self, limit: Optional[int] = None, html_content: Optional[str] = None) -> List[TenderOpportunity]:
        """
        Fetches and parses active Northern Cape Department of Roads and Public Works tenders.
        ...
        if html_content is None:
            try:
                ...
                logger.warning("Failed to fetch Northern Cape tenders live from %s (%s). Engaging high-fidelity local fallback.", self.url, exc)
                html_content = MOCK_NC_ROADS_HTML
        ...
        if not tenders and not engaged_fallback:
            logger.info("Northern Cape live page parsing yielded 0 results. Engaging high-fidelity local fallback.")
            tenders = self.parse_html(MOCK_NC_ROADS_HTML, limit)

        return tenders

    def parse_html(self, html: str, limit: Optional[int] = None) -> List[TenderOpportunity]:
        """
        Parses Northern Cape Department of Roads and Public Works tenders table/list from HTML using standard Python regex.
        """
        tenders = []
        ...
        for row_html in rows:
            ...
            location_target = province_from_text(combined_text) or "Northern Cape"  # Default to Northern Cape
            closing_date = parse_closing_date(closing_str)
            
            opportunity = TenderOpportunity(
                tender_id=ref,
                title=f"{ref}: {title_desc}"[:500],
                issuing_entity="Northern Cape Department of Roads and Public Works",
