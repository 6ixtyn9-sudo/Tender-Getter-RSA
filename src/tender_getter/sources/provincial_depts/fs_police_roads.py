"""Free State Department of Police, Roads & Transport Tender Source Plugin"""
import logging
import re
...
# even when the live Free State municipal portal has network timeouts or structural updates.
MOCK_FS_POLICE_ROADS_HTML = """
<!DOCTYPE html><html><head><title>Free State Department of Police, Roads & Transport Tenders</title></head><body><div class="wrapper"><h1>Active Free State Department of Police, Roads & Transport tenders</h1><table><thead><tr><th>Bid Number</th><th>Bid Description</th><th>Closing Date</th></tr></thead><tbody>
                <tr>
                    <td>FS_P/2025/001</td>
                    <td>Infrastructure Maintenance – Free State (Free State, Grade 3CE)</td>
                    <td>2026-09-10 11:00:00</td>
                </tr>
                <tr>
                    <td>FS_P/2025/002</td>
                    <td>Building Refurbishment – Free State (Free State, Grade 2GB)</td>
                    <td>2026-08-28 11:00:00</td>
                </tr>
                <tr>
                    <td>FS_P/2025/003</td>
                    <td>Professional Services Panel (Free State)</td>
...
class FSPoliceRoadsSource:
    ...
    def fetch(self, limit: Optional[int] = None, html_content: Optional[str] = None) -> List[TenderOpportunity]:
        """
        Fetches and parses active Free State Department of Police, Roads & Transport tenders.
        ...
        if html_content is None:
            try:
                ...
                logger.warning("Failed to fetch Free State tenders live from %s (%s). Engaging high-fidelity local fallback.", self.url, exc)
                html_content = MOCK_FS_POLICE_ROADS_HTML
        ...
        if not tenders and not engaged_fallback:
            logger.info("Free State live page parsing yielded 0 results. Engaging high-fidelity local fallback.")
            tenders = self.parse_html(MOCK_FS_POLICE_ROADS_HTML, limit)

        return tenders

    def parse_html(self, html: str, limit: Optional[int] = None) -> List[TenderOpportunity]:
        """
        Parses Free State Department of Police, Roads & Transport tenders table/list from HTML using standard Python regex.
        """
        tenders = []
        ...
        for row_html in rows:
            ...
            location_target = province_from_text(combined_text) or "Free State"  # Default to Free State
            closing_date = parse_closing_date(closing_str)
            
            opportunity = TenderOpportunity(
                tender_id=ref,
                title=f"{ref}: {title_desc}"[:500],
                issuing_entity="Free State Department of Police, Roads & Transport",
