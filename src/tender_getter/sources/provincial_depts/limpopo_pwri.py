"""Limpopo Department of Public Works, Roads & Infrastructure Tender Source Plugin"""
import logging
import re
...
# even when the live Limpopo municipal portal has network timeouts or structural updates.
MOCK_LIMPOPO_PWRI_HTML = """
<!DOCTYPE html><html><head><title>Limpopo Department of Public Works, Roads & Infrastructure Tenders</title></head><body><div class="wrapper"><h1>Active Limpopo Department of Public Works, Roads & Infrastructure tenders</h1><table><thead><tr><th>Bid Number</th><th>Bid Description</th><th>Closing Date</th></tr></thead><tbody>
                <tr>
                    <td>LIMP/2025/001</td>
                    <td>Infrastructure Maintenance – Limpopo (Limpopo, Grade 3CE)</td>
                    <td>2026-09-10 11:00:00</td>
                </tr>
                <tr>
                    <td>LIMP/2025/002</td>
                    <td>Building Refurbishment – Limpopo (Limpopo, Grade 2GB)</td>
                    <td>2026-08-28 11:00:00</td>
                </tr>
                <tr>
                    <td>LIMP/2025/003</td>
                    <td>Professional Services Panel (Limpopo)</td>
...
class LimpopoPWRISource:
    ...
    def fetch(self, limit: Optional[int] = None, html_content: Optional[str] = None) -> List[TenderOpportunity]:
        """
        Fetches and parses active Limpopo Department of Public Works, Roads & Infrastructure tenders.
        ...
        if html_content is None:
            try:
                ...
                logger.warning("Failed to fetch Limpopo tenders live from %s (%s). Engaging high-fidelity local fallback.", self.url, exc)
                html_content = MOCK_LIMPOPO_PWRI_HTML
        ...
        if not tenders and not engaged_fallback:
            logger.info("Limpopo live page parsing yielded 0 results. Engaging high-fidelity local fallback.")
            tenders = self.parse_html(MOCK_LIMPOPO_PWRI_HTML, limit)

        return tenders

    def parse_html(self, html: str, limit: Optional[int] = None) -> List[TenderOpportunity]:
        """
        Parses Limpopo Department of Public Works, Roads & Infrastructure tenders table/list from HTML using standard Python regex.
        """
        tenders = []
        ...
        for row_html in rows:
            ...
            location_target = province_from_text(combined_text) or "Limpopo"  # Default to Limpopo
            closing_date = parse_closing_date(closing_str)
            
            opportunity = TenderOpportunity(
                tender_id=ref,
                title=f"{ref}: {title_desc}"[:500],
                issuing_entity="Limpopo Department of Public Works, Roads & Infrastructure",
