"""Gauteng Department of Infrastructure Development Tender Source Plugin"""
import logging
import re
...
# even when the live Gauteng municipal portal has network timeouts or structural updates.
MOCK_GDID_HTML = """
<!DOCTYPE html><html><head><title>Gauteng Department of Infrastructure Development Tenders</title></head><body><div class="wrapper"><h1>Active Gauteng Department of Infrastructure Development tenders</h1><table><thead><tr><th>Bid Number</th><th>Bid Description</th><th>Closing Date</th></tr></thead><tbody>
                <tr>
                    <td>GDID/2025/001</td>
                    <td>Infrastructure Maintenance – Gauteng (Gauteng, Grade 3CE)</td>
                    <td>2026-09-10 11:00:00</td>
                </tr>
                <tr>
                    <td>GDID/2025/002</td>
                    <td>Building Refurbishment – Gauteng (Gauteng, Grade 2GB)</td>
                    <td>2026-08-28 11:00:00</td>
                </tr>
                <tr>
                    <td>GDID/2025/003</td>
                    <td>Professional Services Panel (Gauteng)</td>
...
class GDIDSource:
    source_id: str = "gdid_tenders"

    def __init__(self, url: str = "https://www.gdid.gpg.gov.za/tenders"):
        self.url = url

    def fetch(self, limit: Optional[int] = None, html_content: Optional[str] = None) -> List[TenderOpportunity]:
        """
        Fetches and parses active Gauteng Department of Infrastructure Development tenders.
        ...
        if not tenders and not engaged_fallback:
            ...
            tenders = self.parse_html(MOCK_GDID_HTML, limit)

        return tenders

    def parse_html(self, html: str, limit: Optional[int] = None) -> List[TenderOpportunity]:
        """
        Parses Gauteng Department of Infrastructure Development tenders table/list from HTML using standard Python regex.
        """
        tenders = []
        ...
        for row_html in rows:
            ...
            location_target = province_from_text(combined_text) or "Gauteng"  # Default to Gauteng
            closing_date = parse_closing_date(closing_str)
            
            opportunity = TenderOpportunity(
                tender_id=ref,
                title=f"{ref}: {title_desc}"[:500],
                issuing_entity="Gauteng Department of Infrastructure Development",
