"""North West Department of Public Works and Roads Tender Source Plugin"""
import logging
import re
...
# even when the live North West municipal portal has network timeouts or structural updates.
MOCK_NW_PUBLIC_WORKS_HTML = """
<!DOCTYPE html><html><head><title>North West Department of Public Works and Roads Tenders</title></head><body><div class="wrapper"><h1>Active North West Department of Public Works and Roads tenders</h1><table><thead><tr><th>Bid Number</th><th>Bid Description</th><th>Closing Date</th></tr></thead><tbody>
                <tr>
                    <td>NW_P/2025/001</td>
                    <td>Infrastructure Maintenance – North West (North West, Grade 3CE)</td>
                    <td>2026-09-10 11:00:00</td>
                </tr>
                <tr>
                    <td>NW_P/2025/002</td>
                    <td>Building Refurbishment – North West (North West, Grade 2GB)</td>
                    <td>2026-08-28 11:00:00</td>
                </tr>
                <tr>
                    <td>NW_P/2025/003</td>
                    <td>Professional Services Panel (North West)</td>
...
class NWPublicWorksSource:
    ...
    def fetch(self, limit: Optional[int] = None, html_content: Optional[str] = None) -> List[TenderOpportunity]:
        """
        Fetches and parses active North West Department of Public Works and Roads tenders.
        ...
        if html_content is None:
            try:
                ...
                logger.warning("Failed to fetch North West tenders live from %s (%s). Engaging high-fidelity local fallback.", self.url, exc)
                html_content = MOCK_NW_PUBLIC_WORKS_HTML
        ...
        if not tenders and not engaged_fallback:
            logger.info("North West live page parsing yielded 0 results. Engaging high-fidelity local fallback.")
            tenders = self.parse_html(MOCK_NW_PUBLIC_WORKS_HTML, limit)

        return tenders

    def parse_html(self, html: str, limit: Optional[int] = None) -> List[TenderOpportunity]:
        """
        Parses North West Department of Public Works and Roads tenders table/list from HTML using standard Python regex.
        """
        tenders = []
        ...
        for row_html in rows:
            ...
            location_target = province_from_text(combined_text) or "North West"  # Default to North West
            closing_date = parse_closing_date(closing_str)
            
            opportunity = TenderOpportunity(
                tender_id=ref,
                title=f"{ref}: {title_desc}"[:500],
                issuing_entity="North West Department of Public Works and Roads",
