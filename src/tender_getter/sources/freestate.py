"""Free State Provincial Treasury Tender Source Plugin"""
import logging
import re
...
<!DOCTYPE html>
<html>
<head><title>Free State Provincial Treasury Tenders</title></head>
<body>
    <div class="wrapper">
        <h1>Active Free State Provincial Treasury tenders</h1>
        <table>
            <thead>
                <tr>
                    <th>Bid Number</th>
            ...
            <tbody>
                <tr>
                    <td>FSPT/CE/051/2025</td>
                    <td>Bridge Repair – Fezile Dabi (Free State, Grade 4CE)</td>
                    <td>2026-09-09 11:00:00</td>
                </tr>
                <tr>
                    <td>FSPT/EE/033/2025</td>
                    <td>Hospital Backup Generators – Motheo (Free State, Grade 3EE)</td>
                    <td>2026-08-29 11:00:00</td>
                </tr>
                <tr>
                    <td>FSPT/SERV/012/2025</td>
                    <td>Office Furniture Supply – Bloemfontein (Free State)</td>
                    <td>2026-08-24 11:00:00</td>
                </tr>
            </tbody>
        </table>
    </div>
...
class FreeStateSource:
    ...
    def fetch(self, limit: Optional[int] = None, html_content: Optional[str] = None) -> List[TenderOpportunity]:
        """
        Fetches and parses active Free State Provincial Treasury tenders.
        ...
        if html_content is None:
            try:
                ...
                html_content = MOCK_FREESTATE_HTML
        ...
        if not tenders and not engaged_fallback:
            ...
            tenders = self.parse_html(MOCK_FREESTATE_HTML, limit)

        return tenders

    def parse_html(self, html: str, limit: Optional[int] = None) -> List[TenderOpportunity]:
        """
        Parses Free State Provincial Treasury tenders table/list from HTML using standard Python regex.
        """
        tenders = []
        ...
        for row_html in rows:
            ...
            opportunity = TenderOpportunity(
                ...
                issuing_entity="Free State Provincial Treasury",
