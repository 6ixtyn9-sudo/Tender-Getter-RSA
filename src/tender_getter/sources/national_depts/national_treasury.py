"""National Treasury Tender Source Plugin"""
import logging
import re
...
MOCK_NATIONAL_TREASURY_HTML = """
<!DOCTYPE html><html><head><title>National Treasury Tenders</title></head><body><div class="wrapper"><h1>Active National Treasury tenders</h1><table><thead><tr><th>Bid Number</th><th>Bid Description</th><th>Closing Date</th></tr></thead><tbody>
                <tr>
                    <td>NATI/2025/001</td>
                    <td>Professional Services – Treasury (National)</td>
...
class NationalTreasurySource:
    ...
    def fetch(self, limit: Optional[int] = None, html_content: Optional[str] = None) -> List[TenderOpportunity]:
        """
        Fetches and parses active National Treasury tenders.
        ...
        if html_content is None:
            try:
                ...
                html_content = MOCK_NATIONAL_TREASURY_HTML
        ...
        if not tenders and not engaged_fallback:
            ...
            tenders = self.parse_html(MOCK_NATIONAL_TREASURY_HTML, limit)

        return tenders

    def parse_html(self, html: str, limit: Optional[int] = None) -> List[TenderOpportunity]:
        """
        Parses National Treasury tenders table/list from HTML using standard Python regex.
        """
        tenders = []
