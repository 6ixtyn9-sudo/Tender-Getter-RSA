"""The Presidency Tender Source Plugin"""
import logging
import re
...
MOCK_PRESIDENCY_HTML = """
<!DOCTYPE html><html><head><title>The Presidency Tenders</title></head><body><div class="wrapper"><h1>Active The Presidency tenders</h1><table><thead><tr><th>Bid Number</th><th>Bid Description</th><th>Closing Date</th></tr></thead><tbody>
                <tr>
                    <td>PRES/2025/001</td>
                    <td>Professional Services – Presidency (National)</td>
...
class PresidencySource:
    ...
    def fetch(self, limit: Optional[int] = None, html_content: Optional[str] = None) -> List[TenderOpportunity]:
        """
        Fetches and parses active The Presidency tenders.
        ...
        if html_content is None:
            try:
                ...
                html_content = MOCK_PRESIDENCY_HTML
        ...
        if not tenders and not engaged_fallback:
            ...
            tenders = self.parse_html(MOCK_PRESIDENCY_HTML, limit)

        return tenders

    def parse_html(self, html: str, limit: Optional[int] = None) -> List[TenderOpportunity]:
        """
        Parses The Presidency tenders table/list from HTML using standard Python regex.
        """
        tenders = []
