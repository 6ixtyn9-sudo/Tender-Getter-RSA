"""Department of Defence Tender Source Plugin"""
import logging
import re
...
MOCK_DOD_HTML = """
<!DOCTYPE html><html><head><title>Department of Defence Tenders</title></head><body><div class="wrapper"><h1>Active Department of Defence tenders</h1><table><thead><tr><th>Bid Number</th><th>Bid Description</th><th>Closing Date</th></tr></thead><tbody>
                <tr>
                    <td>DOD/2025/001</td>
                    <td>Professional Services – Defence (National)</td>
...
class DoDSource:
    source_id: str = "dod_tenders"

    def __init__(self, url: str = "https://www.dod.mil.za/tenders"):
        self.url = url

    def fetch(self, limit: Optional[int] = None, html_content: Optional[str] = None) -> List[TenderOpportunity]:
        """
        Fetches and parses active Department of Defence tenders.
        ...
        if not tenders and not engaged_fallback:
            ...
            tenders = self.parse_html(MOCK_DOD_HTML, limit)

        return tenders

    def parse_html(self, html: str, limit: Optional[int] = None) -> List[TenderOpportunity]:
        """
        Parses Department of Defence tenders table/list from HTML using standard Python regex.
        """
        tenders = []
        ...
        for row_html in rows:
            ...
            opportunity = TenderOpportunity(
                ...
                issuing_entity="Department of Defence",
