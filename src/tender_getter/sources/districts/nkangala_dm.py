"""Nkangala District Municipality tender source plug-in."""
import logging
import re
...
<!DOCTYPE html>
<html>
<head><title>Nkangala District Municipality Tenders</title></head>
<body>
    <div class="tenders-wrapper">
        <h1>Active Nkangala District Municipality tenders</h1>
        <table>
            <thead>
...
class NkangalaDmSource:
    """Tender source plug-in for Nkangala District Municipality."""

    source_id: str = "nkangala_dm"
    live: bool = True

    def __init__(self, url: str = "https://www.nkangaladm.gov.za/tenders"):
        self.url = url
        self.issuing_entity = "Nkangala District Municipality"
    ...
    def parse_html(self, html: str, limit: Optional[int] = None) -> List[TenderOpportunity]:
        ...
        for row in rows:
            ...
            # Nkangala often leaves reference numbers blank. Use a generated one based on the title.
            ref = "NKANGALA_" + re.sub(r'\W+', '_', title[:20]).strip('_').upper()
