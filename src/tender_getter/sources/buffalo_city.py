"""Buffalo City Metropolitan Municipality Tender Source Plugin"""
import logging
import re
...
MOCK_BUFFALOCITY_HTML = """
<!DOCTYPE html>
<html>
<head><title>Buffalo City Metropolitan Municipality Tenders</title></head>
<body>
    <div class="wrapper">
        <h1>Active Buffalo City Metropolitan Municipality tenders</h1>
        <table>
            <thead>
                <tr>
                    <th>Bid Number</th>
            ...
            <tbody>
                <tr>
                    <td>BCMM/CE/045/2025</td>
                    <td>Rural Gravel Road Upgrade – Mdantsane (Eastern Cape, Grade 4CE)</td>
                    <td>2026-09-12 10:30:00</td>
                </tr>
                <tr>
                    <td>BCMM/EE/012/2025</td>
                    <td>Electrical Network Maintenance – East London (Eastern Cape, Grade 3EE)</td>
                    <td>2026-08-31 10:30:00</td>
                </tr>
                <tr>
                    <td>BCMM/GB/007/2025</td>
                    <td>Community Hall Construction – King Williams Town (Eastern Cape, Grade 2GB)</td>
                    <td>2026-08-25 10:30:00</td>
                </tr>
            </tbody>
        </table>
    </div>
...
class BuffaloCitySource:
    ...
    def fetch(self, limit: Optional[int] = None, html_content: Optional[str] = None) -> List[TenderOpportunity]:
        """
        Fetches and parses active Buffalo City Metropolitan Municipality tenders.
        ...
        if html_content is None:
            try:
                ...
                html_content = MOCK_BUFFALOCITY_HTML
        ...
        if not tenders and not engaged_fallback:
            ...
            tenders = self.parse_html(MOCK_BUFFALOCITY_HTML, limit)

        return tenders

    def parse_html(self, html: str, limit: Optional[int] = None) -> List[TenderOpportunity]:
        """
        Parses Buffalo City Metropolitan Municipality tenders table/list from HTML using standard Python regex.
        """
        tenders = []
        ...
        for row_html in rows:
            ...
            location_target = province_from_text(combined_text) or "Eastern Cape"  # Default to Eastern Cape for BuffaloCity
            closing_date = parse_closing_date(closing_str)
            
            opportunity = TenderOpportunity(
                tender_id=ref,
                title=f"{ref}: {title_desc}"[:500],
                issuing_entity="Buffalo City Metropolitan Municipality",
