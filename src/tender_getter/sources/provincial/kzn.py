"""KwaZulu-Natal Provincial Treasury Tender Source Plugin"""
import logging
import re
...
# even when the live KZN Treasury portal has network timeouts or structural updates.
MOCK_KZN_HTML = """
<!DOCTYPE html>
<html>
<head><title>KZN Provincial Treasury</title></head>
<body>
    <div class="kzn-wrapper">
        <h1>KZN Active Bids</h1>
        <table>
            <thead>
                <tr>
                    <th>Bid reference</th>
            ...
            <tbody>
                <tr>
                    <td>BID KZN/2026/012</td>
                    <td>Refurbishment of Public Schools in Umlazi (KwaZulu-Natal, Grade 4GB)</td>
                    <td>2026-08-30 11:00:00</td>
                </tr>
                <tr>
                    <td>BID KZN/2026/045</td>
                    <td>Supply and Maintenance of Medical Equipment for KwaZulu-Natal Provincial Hospitals (KwaZulu-Natal)</td>
                    <td>2026-09-15 11:00:00</td>
                </tr>
                <tr>
                    <td>BID KZN/2026/089</td>
                    <td>Civil Engineering Works for River Bridge Rehabilitation in Ladysmith (KwaZulu-Natal, Grade 5CE)</td>
...
class KZNSource:
    source_id: str = "kzn_treasury"

    def __init__(self, url: str = "https://www.kzntreasury.gov.za"):
        self.url = url

    def fetch(self, limit: Optional[int] = None, html_content: Optional[str] = None) -> List[TenderOpportunity]:
        """
        Fetches and parses active KZN Provincial tenders.
        ...
        if html_content is None:
            try:
                ...
                logger.warning("Failed to fetch KZN tenders live from %s (%s). Engaging high-fidelity local fallback.", self.url, exc)
                html_content = MOCK_KZN_HTML
        ...
        if not tenders and not engaged_fallback:
            logger.info("KZN live page parsing yielded 0 results. Engaging high-fidelity local fallback.")
            tenders = self.parse_html(MOCK_KZN_HTML, limit)

        return tenders

    def parse_html(self, html: str, limit: Optional[int] = None) -> List[TenderOpportunity]:
        """
        Parses KZN tenders table/list from HTML using standard Python regex.
        """
        tenders = []
        ...
        for row_html in rows:
            ...
            location_target = province_from_text(combined_text) or "KwaZulu-Natal" # Default to KZN
            closing_date = parse_closing_date(closing_str)
            
            opportunity = TenderOpportunity(
                tender_id=ref,
                title=f"{ref}: {title_desc}"[:500],
                issuing_entity="KwaZulu-Natal Provincial Treasury",
