"""City of Cape Town Metropolitan Municipality Tender Source Plugin"""
import logging
import re
...
# even when the live Cape Town municipal portal has network timeouts or structural updates.
MOCK_CAPETOWN_HTML = """
<!DOCTYPE html>
<html>
<head><title>City of Cape Town Tenders</title></head>
<body>
    <div class="coct-wrapper">
        <h1>Active City of Cape Town tenders</h1>
        <table>
            <thead>
                <tr>
                    <th>Tender Number</th>
            ...
            <tbody>
                <tr>
                    <td>282S/2025/26</td>
                    <td>Provision of Electrical Grid Maintenance in Khayelitsha (Western Cape, Grade 3EE)</td>
                    <td>2026-08-30 11:00:00</td>
                </tr>
                <tr>
                    <td>119Q/2025/26</td>
                    <td>Construction of Sewer and Stormwater Infrastructure in Mitchells Plain (Western Cape, Grade 6CE)</td>
                    <td>2026-09-15 11:00:00</td>
                </tr>
                <tr>
                    <td>305G/2025/26</td>
                    <td>Supply and Delivery of Water Meters for Cape Town Water Department (Western Cape)</td>
...
class CapeTownSource:
    ...
    def fetch(self, limit: Optional[int] = None, html_content: Optional[str] = None) -> List[TenderOpportunity]:
        """
        Fetches and parses active City of Cape Town tenders.
        ...
        if html_content is None:
            try:
                ...
                logger.warning("Failed to fetch Cape Town tenders live from %s (%s). Engaging high-fidelity local fallback.", self.url, exc)
                html_content = MOCK_CAPETOWN_HTML
        ...
        if not tenders and not engaged_fallback:
            logger.info("Cape Town live page parsing yielded 0 results. Engaging high-fidelity local fallback.")
            tenders = self.parse_html(MOCK_CAPETOWN_HTML, limit)

        return tenders

    def parse_html(self, html: str, limit: Optional[int] = None) -> List[TenderOpportunity]:
        """
        Parses City of Cape Town tenders table/list from HTML using standard Python regex.
        """
        tenders = []
        ...
        for row_html in rows:
            ...
            location_target = province_from_text(combined_text) or "Western Cape" # Default to Western Cape for CoCT
            closing_date = parse_closing_date(closing_str)
            
            opportunity = TenderOpportunity(
                tender_id=ref,
                title=f"{ref}: {title_desc}"[:500],
                issuing_entity="City of Cape Town Metropolitan Municipality",
