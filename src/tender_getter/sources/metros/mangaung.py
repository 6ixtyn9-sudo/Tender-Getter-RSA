"""Mangaung Metropolitan Municipality Tender Source Plugin"""
import logging
import re
...
# even when the live Mangaung Metropolitan Municipality municipal portal has network timeouts or structural updates.
MOCK_MANGAUNG_HTML = """
<!DOCTYPE html>
<html>
<head><title>Mangaung Metropolitan Municipality Tenders</title></head>
<body>
    <div class="wrapper">
        <h1>Active Mangaung Metropolitan Municipality tenders</h1>
        <table>
            <thead>
                <tr>
                    <th>Bid Number</th>
            ...
            <tbody>
                <tr>
                    <td>MMM/CE/2025/018</td>
                    <td>Sewer Pipeline Replacement – Bloemfontein (Free State, Grade 5CE)</td>
                    <td>2026-09-15 10:30:00</td>
                </tr>
                <tr>
                    <td>MMM/EE/2025/004</td>
                    <td>High Mast Light Installation – Botshabelo (Free State, Grade 3EE)</td>
                    <td>2026-08-28 10:30:00</td>
                </tr>
                <tr>
                    <td>MMM/GB/2025/009</td>
                    <td>Municipal Office Refurbishment (Free State, Grade 2GB)</td>
                    <td>2026-08-22 10:30:00</td>
                </tr>
            </tbody>
        </table>
    </div>
...
class MangaungSource:
    ...
    def fetch(self, limit: Optional[int] = None, html_content: Optional[str] = None) -> List[TenderOpportunity]:
        """
        Fetches and parses active Mangaung Metropolitan Municipality tenders.
        ...
        if html_content is None:
            try:
                ...
                logger.warning("Failed to fetch Mangaung Metropolitan Municipality tenders live from %s (%s). Engaging high-fidelity local fallback.", self.url, exc)
                html_content = MOCK_MANGAUNG_HTML
        ...
        if not tenders and not engaged_fallback:
            logger.info("Mangaung Metropolitan Municipality live page parsing yielded 0 results. Engaging high-fidelity local fallback.")
            tenders = self.parse_html(MOCK_MANGAUNG_HTML, limit)

        return tenders

    def parse_html(self, html: str, limit: Optional[int] = None) -> List[TenderOpportunity]:
        """
        Parses Mangaung Metropolitan Municipality tenders table/list from HTML using standard Python regex.
        """
        tenders = []
        ...
        for row_html in rows:
            ...
            location_target = province_from_text(combined_text) or "Free State"  # Default to Free State for Mangaung
            closing_date = parse_closing_date(closing_str)
            
            opportunity = TenderOpportunity(
                tender_id=ref,
                title=f"{ref}: {title_desc}"[:500],
                issuing_entity="Mangaung Metropolitan Municipality",
